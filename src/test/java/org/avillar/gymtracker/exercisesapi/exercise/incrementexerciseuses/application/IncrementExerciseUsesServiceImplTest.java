...
class IncrementExerciseUsesServiceImplTest {

  private final EasyRandom easyRandom = new EasyRandom();

  @InjectMocks private IncrementExerciseUsesServiceImpl incrementExerciseUsesService;
  ...
  @Test
  void incrementOk() {
    final int uses = getNotNegativeRandomInt();
    ...
    final IncrementExerciseUsesResponseApplication result =
        incrementExerciseUsesService.execute(exercise.getId());
  ...
  void incrementFirstTimeOk() {
    ...
    final IncrementExerciseUsesResponseApplication result =
        incrementExerciseUsesService.execute(exercise.getId());
    assertThat(result).isNotNull();
    assertThat(result.getUses()).isEqualTo(1);

    final ExerciseUses exerciseUsesSaved = exerciseUsesArgumentCaptor.getValue();
    assertThat(exerciseUsesSaved).isNotNull();
    assertThat(exerciseUsesSaved.getUses()).isEqualTo(1);
    assertThat(exerciseUsesSaved.getExercise()).isEqualTo(exercise);
  ...
  void incrementKoExerciseNotFound() {
    ...
    final IllegalAccessException exception =
        assertThrows(
            IllegalAccessException.class, () -> incrementExerciseUsesService.execute(exerciseId));
  ...
  private int getNotNegativeRandomInt() {
    return Math.max(0, easyRandom.nextInt());
  }
}
